...
- **Enhanced Connection Resilience**: All core API interactions now feature **increased timeouts (30s)** and robust connection management. Quota monitoring now implements **Proxy Session Rotation**, generating a fresh session ID on every retry to rotate exit IPs and bypass bad proxy nodes or "SSL record layer failure" errors. Administrative tasks like Order Sync now respect granular proxy settings to bypass server-side rate limits and IP blocks.
- **Modernized Stealth & Compatibility**: All components now use a standardized, stable **Firefox 132.0** User-Agent. The API client has been refactored to recreate the entire HTTP session upon detecting network failures, ensuring maximum reliability against persistent connection glitches.
- **Profile Usage Limits**: Enforces a fair-usage limit of 3 registration profiles per regular user to ensure system stability and performance (**Admin users are exempt** and their profiles are automatically prioritized at the top of every registration batch).
...
- **Hierarchical Admin Dashboard**: Includes a decluttered, hidden admin panel organized into logical submenus: **Users Control**, **Profiles Control**, **Infrastructure & Proxy**, and **Admin Inbox**. Features toggleable **"Restricted Mode"** (locks non-essential commands), **"Private Mode"** (locks the entire bot to admin-only), **live concurrency limit adjustment**, **live check interval updates**, **granular proxy controls**, and **recent quota event history**.
- **Private Mode (Admin-Only)**: A high-security toggle that immediately stops all public users from interacting with the bot. When enabled, any non-admin interaction receives a localized message explaining that the bot is for testing only. Perfect for maintenance windows or private deployments.
...
- **Intelligent UX Guardrails**: Prevents avoidable registration failures by implementing proactive input normalization and validation. The bot now intelligently recognizes and filters out common "skip" keywords (e.g., "skip", "aucun", "none", "لا") in multiple languages for optional fields like email. Additionally, it features **real-time MICLAT error detection**, immediately notifying users in their preferred language if the server rejects a profile due to an invalid NIN not found in the Ministry of Interior's database.
...
- **Admin Error & Warning Inbox**: A centralized, persistent monitoring system that intercepts all `ERROR` and `WARNING` log events. Features real-time Telegram notifications for admins, a filterable paginated dashboard (by level, status, and date range), and a resolution tracking system.
- **Inbox Notification Controls**: To prevent Telegram rate-limiting or "clogging" during error bursts, admins can toggle between **Real-time Alerts** and **Periodic Summary Reports**. When muted, the bot collects unresolved issues and sends a consolidated statistics report at a user-defined interval (e.g., every 60 minutes).
- **Soft-Delete Inbox Clearing**: Includes a **Clear Inbox** feature that hides all current entries from the dashboard to maintain a clean workspace, while preserving the raw logs in the database for future debugging or historical audits.
- **Admin Audit & Sync Suite**: Includes a comprehensive set of maintenance tools:
  - **Order Sync Reconciliation**: Batch checks all registered profiles against the server to find and flag missing orders.
  ...
  - **Invalid NIN Notifier**: A batch maintenance tool that scans the Error Inbox for past 'MICLAT NOT FOUND' rejections and notifies affected users to correct their data.
...
- **Scheduler Congestion Resilience**: Optimized the core polling loop to prevent "skipped execution" and noisy warnings during high-traffic periods:
  - **Silent Job Skipping**: Implemented an `asyncio.Lock` mechanism that allows the scheduler to skip overlapping runs silently, keeping the logs clean and ensuring only one poll is active at a time.
  - **Increased Concurrency**: Configured `max_instances=5` to allow the scheduler to queue checks during transient API slowness without dropping cycles.
