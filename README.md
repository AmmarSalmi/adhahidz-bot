...
- **Global Profile Synchronization**: A scalable, rate-limit-aware administrative system that performs a two-stage cascade audit of all registration profiles. Stage 1 (Light Probe) uses rapid OTP resending to identify active accounts, while Stage 2 (Deep Audit) performs full login and order verification. It automatically reconciles local data, marks secured orders, and notifies users with localized updates.
...
- **Improved Log Stream**: Suppresses verbose library success logs (HTTP 200s) and silences noisy tracebacks for common transient events (like **Telegram API timeouts/congestion** or users blocking the bot) to keep the log stream high-signal and actionable.
